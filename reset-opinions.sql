...
-- Generated by scratch-build-reset.mjs; DDL blocks are copied verbatim from
-- create_new.sql so they cannot drift from the authority.
--
-- KEEP (not touched): persons, refs, authorities -- the already-migrated input layer.
--
-- AFTER running this you MUST re-mint the name_opinions ROOT rows from authorities
-- (edge_class='root') BEFORE running the opinions migration -- it reads those permids.
-- (See the note in the accompanying message: migrate-name-opinions.js currently
-- targets the older schema and needs a small update first.)
