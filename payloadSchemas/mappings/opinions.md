...
|''| != `original spelling` ? add additional record | NA | name_opinions | name | reason from namechange_reasons record that is closest match to spelling_reason (use "rank change" = "reranked", "reassignment" = "assignment") | false | NA | permid of name_opinions record with oldpbdbid = child_spelling_no | permid of name_opinions record with oldpbdbid = child_no|
...
|''| != `original spelling` ? add additional record | NA | name_opinions | name | reason from namechange_reasons record that is closest match to spelling_reason (use "rank change" = "reranked", "reassignment" = "assignment") | NA | NA | permid of name_opinions record with oldpbdbid = child_spelling_no | permid of name_opinions record with oldpbdbid = child_no|
...
|''| != `original spelling` ? add additional record | NA | name_opinions | name | reason from namechange_reasons record that is closest match to spelling_reason (use "rank change" = "reranked", "reassignment" = "assignment") | NA | NA | permid of name_opinions record with oldpbdbid = child_spelling_no | permid of name_opinions record with oldpbdbid = child_no|
|`misspelling of` | (all get this record) | NA | name_opinions | name | historical misspelling | NA | NA |  permid of name_opinions record with oldpbdbid = child_spelling_no | parent_spelling_no = 0 ? NULL : permid of name_opinions record with oldpbdbid = parent_spelling_no|
...
2. **Primary vs. name are resolved and skipped independently.** The primary disposition (assignment /
   concept / validity) and the universal name backfill (the "additional record" rows, `edge_class =
   'name'`) are two independent outputs. Each is resolved, and if necessary skipped-and-logged, on its
...
3. **Self-referential edges are never written.** No output whose `subject_permid` would equal its
   `target_permid` (concept/name edges) or its `containing_permid` (assignment edges) is written. Such
   rows are skipped-and-logged (per output type), never allowed to reach the DB constraint. This covers
   both same-taxon assignment (`child_spelling_no == parent_spelling_no`) and name self-reference
...
5. **Per-output reconciliation.** For each independent output type a pair can produce (primary disposition,
   name backfill), `written + skipped-with-a-logged-reason == source rows read` for that pair. No row is
...
6. **Why `historical misspelling` for `misspelling of`.** The `misspelling of` name edge uses the
   ...
   `misspelling` records one noticed incidentally while entering some other opinion. Its name target is
...
7. **Why `nomen oblitum` branches per row.** `nomen oblitum` carries a real target on some rows and none on
   ...
   (`nomenclatural_status_id → nomen oblitum`). This per-row branch is independent of the name backfill.
...
`child_spelling_no ≠ child_no`, so — against the rule above — they *do* carry a name
claim. Each is backfilled as one extra `name_opinions` name row (`edge_class =
'name'`, `subject = permid(child_spelling_no)`, `target = permid(child_no)`), with the
...
- **Name self-reference** (`child_spelling_no == child_no` despite a non-`original
  spelling` `spelling_reason`): no real deviation to record, so no name edge is emitted.
