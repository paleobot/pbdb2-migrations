...
The script SHALL read all rows from MariaDB `refs` table, plus all rows from `ref_authors` and `ref_editors`. The `ref_authors` and `ref_editors` rows SHALL be pre-loaded into Maps keyed by `reference_no` for lookup during transformation.

#### Scenario: Full extraction
- **WHEN** the migration script executes the source queries
- **THEN** all 93,863 refs rows, 14,144 ref_authors rows, and 414 ref_editors rows are read from MariaDB and logged
...
The script SHALL build a jsonb `authors` array of `{surname, givenName}` objects for each reference. If `ref_authors` entries exist for a `reference_no`, those SHALL be used (ordered by `place`). Otherwise, the script SHALL build authors from the flat fields (`author1init`/`author1last`, `author2init`/`author2last`, `otherauthors`).

#### Scenario: ref_authors entries available
- **WHEN** a reference has entries in `ref_authors`
- **THEN** the `authors` array is built from `ref_authors` ordered by `place`

#### Scenario: Flat fields only
- **WHEN** a reference has no `ref_authors` entries and `author1last = 'Smith'`, `author1init = 'J.'`
- **THEN** the `authors` array contains at least `[{surname: "Smith", givenName: "J."}]`

#### Scenario: otherauthors parsing
- **WHEN** a reference has no `ref_authors` entries and `otherauthors` is non-empty
- **THEN** the script attempts to parse `otherauthors` into additional author entries and logs a warning for unparseable values
...
- `oldpbdbID` from the source `reference_no` (as a string) for traceability
...
#### Scenario: oldpbdbID traceability
- **WHEN** a ref with `reference_no = 5000` is migrated
- **THEN** the jsonb contains `oldpbdbID: "5000"`
...
The script SHALL insert rows in batches (e.g. 500 per batch) using multi-value INSERT with `ON CONFLICT (id) DO UPDATE SET ...`. The `permid` column MUST be excluded from the UPDATE SET to preserve UUIDs on re-runs.
...
- **WHEN** the script is re-run against a populated `references` table
