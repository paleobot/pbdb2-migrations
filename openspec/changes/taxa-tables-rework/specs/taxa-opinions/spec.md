...
- FROM: `### Requirement: The taxa ledger exists as derived output`
  TO: `### Requirement: The taxa_linnaean ledger exists as derived output`
- FROM: `### Requirement: derive_taxa() is a pure function of the opinions`
  TO: `### Requirement: derive_linnaean() is a pure function of the opinions`
- FROM: `### Requirement: derive_taxa() terminates on cycles and surfaces containment cycles`
  TO: `### Requirement: derive_linnaean() terminates on cycles and surfaces containment cycles`
- FROM: `### Requirement: derive_taxa(subset) equals derive_taxa(all) for the requested permids`
  TO: `### Requirement: derive_linnaean(subset) equals derive_linnaean(all) for the requested permids`
- FROM: `### Requirement: derive_taxa() is total over minted permids`
  TO: `### Requirement: derive_linnaean() is total over minted permids`
- FROM: `### Requirement: rebuild_taxa() materializes the ledger and the invariant holds`
  TO: `### Requirement: rebuild_linnaean() materializes the ledger and the invariant holds`
...
- **WHEN** a subject permid has no other current `name`-class opinion besides a newly inserted
  negating one
- **THEN** the insert succeeds, and `derive_linnaean()` treats the subject exactly as if it had no
...
- **THEN** `derive_linnaean()` excludes the subject from any lineage union while still grouping its
  lineage into the concept the winning `concept`-class opinion names

### Requirement: The taxa_linnaean ledger exists as derived output

The schema SHALL define a `taxa_linnaean` table with one row per name-as-spelled permid, holding the immutable denormalized identity (`name text NOT NULL`, `rank_id integer NOT NULL` FK to `dictionaries.taxonomy_ranks`, optional `authority_id`), the derived identity triad (`original_permid`, `accepted_spelling_permid`, `concept_permid`, all `NOT NULL`), classification (`containing_concept_permid` nullable, `classification_path ltree`), `nomenclatural_status_id`, and provenance (`winning_name_opinion_id`, `winning_assignment_opinion_id`, `winning_validity_opinion_id` FKs to the three opinion tables). It SHALL NOT carry `authorizer_person_id`/`enterer_person_id` (no row is hand-entered) and SHALL NOT carry a `winning_rank_opinion_id`.
...
- **WHEN** a `taxa_linnaean` row is inserted with `rank_id IS NULL`
...
`taxon_annotations` SHALL be versioned via `install_version_triggers()` (getting `place_in_lineage()`, `handle_new_version()`, and the automatic `permid` head index). `taxa_linnaean` SHALL NOT be versioned: it carries no `preceded_by_id`/`succeeded_by_id` and is not passed to `install_version_triggers()` — it SHALL instead enforce `UNIQUE (permid)` (one row per permid, updated in place, not a lineage of historical rows). The three opinion tables SHALL be versioned by carrying `permid` + succession columns but SHALL NOT call `install_version_triggers()`, and SHALL instead hand-create their own head-only `permid` indexes.
...
The schema SHALL define `taxon_annotations` (versioned curatorial prose: `common_name`, `comments`, `discussion`, `discussed_by_reference_id`) keyed by `subject_permid`, and `homonyms` grouping permids by an app-minted `homonym_group_id uuid` (uuidv7), with `UNIQUE (homonym_group_id, permid)`. Neither SHALL be reconstructable by `derive()`; `taxa_linnaean` SHALL NOT carry a `has_homonym` flag.
...
### Requirement: derive_linnaean() is a pure function of the opinions

`derive_linnaean(permids)` SHALL compute the accepted name, rank, groupings, classification, and validity of taxa reading **only** the Layer 1 opinion tables (`name_opinions`, `assignment_opinions`, `validity_opinions`) and `refs` (for `pubyr`). It SHALL NOT read the `taxa_linnaean` ledger, and SHALL NOT write to any table. It SHALL consider only current, non-removed assertions (`removed IS NOT TRUE AND succeeded_by_id IS NULL`).

#### Scenario: derive_taxa() output does not depend on the ledger

- **WHEN** `derive_linnaean()` is called after the `taxa_linnaean` ledger is truncated
...
- **THEN** `derive_linnaean()` does not use it in any ranking or grouping
...
- **THEN** `derive_linnaean()` reports the same `concept_permid` for members of J and S, equal to S's accepted spelling
...
- **THEN** `derive_linnaean()` unions L into S's concept, not the lower-ranked opinion's target's concept
...
- **THEN** `derive_linnaean()` reports `concept_permid` for L's members equal to L's own accepted spelling, not S's
...
- **THEN** `derive_linnaean()` excludes that edge from the concept union-find, and the unranked-clade lineage forms its own concept rather than merging into the Linnaean-ranked lineage's concept
...
- **THEN** `derive_linnaean()` still excludes that edge — the exclusion applies whenever either side is unranked/unranked clade, not only when one side is Linnaean-ranked — and each forms its own concept
...
- **THEN** `derive_linnaean()` selects that spelling as `accepted_spelling_permid`
...
- **THEN** `derive_linnaean()` does not select it; the accepted spelling is the top-ranked eligible non-misspelling
...
- **THEN** `derive_linnaean()` uses it to set the concept's `containing_concept_permid`
...
- **THEN** `derive_linnaean()` does not borrow it; the species is placed by its own direct assignment
...
- **THEN** `derive_linnaean()` excludes the self-referential candidate from the ranking contest entirely, and sets `containing_concept_permid` from the top-ranked remaining candidate, by the same `evidence DESC, yr DESC NULLS LAST, opinion_id DESC` order used when no self-reference is present
...
- **THEN** `derive_linnaean()` sets that concept's `containing_concept_permid` to `NULL` rather than raising an error, selecting one of the excluded candidates anyway, or leaving the concept unresolved
...
- **THEN** `derive_linnaean()` excludes that candidate from the pool and sets the concept's `containing_concept_permid` to `NULL` rather than using it
...
- **THEN** `derive_linnaean()` excludes that candidate from the pool, falling through to the next-ranked non-excluded candidate or to `NULL` if none remains
...
- **THEN** `derive_linnaean()` excludes the rank-inverted candidate from the ranking contest entirely, and sets `containing_concept_permid` from the top-ranked remaining candidate
...
- **THEN** `derive_linnaean()` does not exclude that candidate on rank-cardinality grounds; it competes normally in the ranking contest
...
- **THEN** `derive_linnaean()` sets that concept's `containing_concept_permid` to `NULL` rather than raising an error or selecting one of the excluded candidates anyway
...
- **THEN** `derive_linnaean()` picks exactly one of A/B as senior per the tiebreak, and repeated calls return the same choice
...
- **THEN** `derive_linnaean()` raises an error identifying the cycle rather than returning
...
- **THEN** `derive_linnaean()` resolves it to `containing_concept_permid = NULL` via the pooling exclusion, and this concept never triggers the containment-cycle guard
...
### Requirement: derive_linnaean(subset) equals derive_linnaean(all) for the requested permids

`derive_linnaean(permids)` SHALL return, for each requested permid, exactly the row `derive_linnaean(all)` would produce, by internally expanding the seed set to full lineage/concept components before computing.

#### Scenario: Deriving a single junior synonym matches the full derivation

- **WHEN** `derive_linnaean(ARRAY[<one junior-synonym permid>])` is called
- **THEN** that permid's row equals its row from `derive_linnaean(all)` (same `concept_permid`, `accepted_spelling_permid`, `containing_concept_permid`, `classification_path`)
...
- **WHEN** `derive_linnaean(all)` runs over a fixture of N minted permids, none of them exhausted
...
- **THEN** `derive_linnaean()` raises an error identifying the permid, rather than emitting one row per root row or picking one silently

### Requirement: classification_path materializes the concept adjacency

`derive_linnaean()` SHALL produce `classification_path` as an `ltree` of concept permids from root to node, consistent with `containing_concept_permid` (adjacency is primary; the path is derived from it).
...
### Requirement: rebuild_linnaean() materializes the ledger and the invariant holds

`rebuild_linnaean()` SHALL call `derive_linnaean(all)` and load the `taxa_linnaean` ledger by upserting in place: updating a permid's existing row where derived output differs, and inserting a row where the permid is new, recording provenance (`winning_name_opinion_id`, `winning_assignment_opinion_id`, `winning_validity_opinion_id`). A callable check SHALL assert the invariant `derive_linnaean(all) ≡ the current taxa_linnaean rows`.

#### Scenario: After rebuild, derive_taxa(all) equals the ledger heads

- **WHEN** `rebuild_linnaean()` runs over a fixture opinion set
- **THEN** the invariant check reports equality between `derive_linnaean(all)` and the current `taxa_linnaean` rows

#### Scenario: A no-op re-derivation appends no versions

- **WHEN** `rebuild_linnaean()` runs twice with no intervening opinion changes
- **THEN** the second run updates and inserts no rows (output equals the existing `taxa_linnaean` rows)
...
- **THEN** its `taxa_linnaean` row's `winning_assignment_opinion_id` references that opinion
