...
## Purpose
Migrate every legacy MariaDB `authorities` row into the new PostgreSQL `name_opinions` table as a root (minting) edge — self-anchored permid, reason `original`/edge_class `root` — resolving authority/reference/attribution/year provenance through the already-migrated `authorities` records; the 18 `informal`-rank rows additionally emit a `validity_opinions` row (status `informal`).
...
For each of the 18 source rows with `taxon_rank = 'informal'`, in addition to the root `name_opinions` row (whose `rank_id` is `'unranked'`), the script SHALL insert one `validity_opinions` row. It SHALL set: `subject_permid` to that name_opinion's `permid`; `nomenclatural_status_id` to the `dictionaries.nomenclatural_statuses` id whose `status = 'informal'`; `targeted = false`; `target_permid = NULL`; `reference_id`, `attribution`, `publication_year` from the same resolved authority as the name_opinion; `authorizer_person_id`/`enterer_person_id` from the same resolved persons; `evidence = false`; `removed = false`.

#### Scenario: Informal row emits both records
- **WHEN** a source row has `taxon_rank = 'informal'`
- **THEN** a `name_opinions` row with `rank_id = 'unranked'` AND a `validity_opinions` row with `nomenclatural_status_id = 'informal'` are inserted, sharing the same `subject_permid`

#### Scenario: validity target shape satisfied
- **WHEN** the informal `validity_opinions` row is inserted
- **THEN** `targeted = false` and `target_permid IS NULL`, satisfying `validity_target_shape` and the composite `(nomenclatural_status_id, targeted)` FK

#### Scenario: Exactly 18 validity rows
- **WHEN** the migration completes
- **THEN** exactly 18 `validity_opinions` rows exist, one per informal-rank source row
...
The script SHALL wrap the bulk insert of all `name_opinions` and `validity_opinions` rows in a single Postgres transaction (`BEGIN` … `COMMIT`), ordered so each informal `validity_opinions` row's `subject_permid` is available. On any failure before `COMMIT`, Postgres SHALL roll back atomically, leaving both tables in their pre-run state with no manual cleanup required.

#### Scenario: Successful bulk insert
- **WHEN** all ~517,284 name_opinions and 18 validity_opinions rows insert without error
- **THEN** the transaction commits and both tables reflect the inserts
...
- **THEN** the transaction rolls back and no migrated rows remain in either table
...
The script SHALL log total source rows read, name_opinions inserted, validity_opinions inserted, informal-rank rows, and skipped orphan rows, and SHALL assert that inserted name_opinions plus skipped rows equals source rows read.

#### Scenario: Final counts logged
- **WHEN** the script completes
- **THEN** it logs `{sourceRows, nameOpinionsInserted, validityInserted, informalCount, skipped}` and confirms `nameOpinionsInserted + skipped == sourceRows`
