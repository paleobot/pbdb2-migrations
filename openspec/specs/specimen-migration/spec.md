...
- `identifiers.institutionCode` from the joined collection's `museum`. When `museum` holds more than one
  ...
  the sentinel `'none specified'`, which was added to the schema enum for this purpose.
...
The script SHALL validate every built `specimen` payload against `specimenSchema` from
`payloadSchemas/specimen.schema.js` before any database write. A validation failure SHALL log the offending
`specimen_no`, the validation errors, and the payload, and SHALL abort the migration.

Validation is a meaningful gate here because all seven enums the payload carries were verified to match their
classic sources value-for-value, so any enum violation reaching the validator indicates a transformation bug
rather than unexpected source data.
