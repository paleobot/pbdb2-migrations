// Quantifies the blast radius of the NEW "rank cardinality" rule
// (test-rank-cardinality.js), on top of the already-shipped self-reference +
// unranked-rank exclusions. Same before/after methodology as
...
async function buildPipeline(client, { rankCardinality }) {
  await client.query(`
  ...
  const rankCardinalityWhere = rankCardinality
...
async function main() {
  ...
  try {
    console.log(`[${new Date().toISOString()}] Building BEFORE pipeline (shipped fixes only, no rank-cardinality)...`);
    let t0 = process.hrtime.bigint();
    await buildPipeline(client, { rankCardinality: false });
    ...
    console.log(`[${new Date().toISOString()}] Building AFTER pipeline (+ rank-cardinality)...`);
    t0 = process.hrtime.bigint();
    await buildPipeline(client, { rankCardinality: true });
    console.log(`  built in ${ms(process.hrtime.bigint() - t0).toFixed(1)} ms`);
