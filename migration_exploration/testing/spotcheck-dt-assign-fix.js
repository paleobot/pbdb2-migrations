// Task 2.4 spot-check for openspec/changes/fix-dt-assign-containment-cycle:
// rebuilds the FIXED pipeline (with _dt_assign's self-reference exclusion)
...
async function main() {
  ...
  try {
    ...
    // ---- FIXED _dt_assign: self-referential candidates excluded before ranking ----
    await client.query(`
    ...
    console.log('');
    console.log('=== Spot-check: the memory-documented 71/73 example, "Parafollicucullus (Yaoconus)" ===');
    const { rows: pf } = await client.query(`
      SELECT dn.con_rep, dn.concept_permid, dn.containing_concept_permid,
             dn.winning_assignment_opinion_id,
             n1.new_name AS concept_name, n2.new_name AS containing_name
      FROM _dt_node dn
      ...
      WHERE n1.new_name = 'Parafollicucullus (Yaoconus)'
    `);
    for (const r of pf) {
      console.log(`  concept "${r.concept_name}" -> containing_concept_permid=${r.containing_concept_permid ?? 'NULL (rootless)'} ("${r.containing_name ?? 'n/a'}")`);
      console.log(`  winning_assignment_opinion_id=${r.winning_assignment_opinion_id ?? 'NULL (excluded down to nothing)'}`);
      console.log(`  self-referential? ${r.concept_permid === r.containing_concept_permid ? 'YES -- BUG' : 'no'}`);
    }
    if (pf.length === 0) console.log('  (not found by that exact name -- may have moved lineage/concept representative)');

    console.log('');
    console.log('=== Sanity: any remaining direct self-loops at all post-fix? ===');
    const { rows: check } = await client.query(`
      SELECT count(*) FROM _dt_node WHERE containing_concept_permid = concept_permid
    `);
    console.log(`  Direct self-loops: ${check[0].count}`);

    console.log('');
    console.log('=== How many concepts are now rootless (containing_concept_permid IS NULL)? ===');
    const { rows: rootless } = await client.query(`
      SELECT count(*) AS total_rootless,
             count(*) FILTER (WHERE dn.winning_assignment_opinion_id IS NULL) AS rootless_no_assign_row_at_all
      FROM _dt_node dn
    `);
    console.log(`  ${JSON.stringify(rootless[0])}`);
  } finally {
    client.release();
    await closePgPlay();
  }
