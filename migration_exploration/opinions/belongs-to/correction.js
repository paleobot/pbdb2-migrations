// Pair: status = 'belongs to', spelling_reason = 'correction' (9,659 rows).
//
...
//
// Every row emits BOTH a name_opinions lineage row (reason 'correction', per the §6.1
// crosswalk for spelling_reason='correction') and an assignment_opinions containment
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting belongs-to/correction migration...`);
  ...
  if (reasonRows.length !== 1) {
    console.error(`  FATAL: expected exactly one namechange_reasons row for reason='correction'/edge_class='name', got ${reasonRows.length}`);
    process.exit(1);
  }
  const correctionReasonId = reasonRows[0].id;

  let sourceRows = 0;
  let orphanReference = 0;
  let childSpellingUnresolved = 0;
  let assignRootless = 0;
  const assignSkip = { parent_spelling_orphan: 0, self_reference: 0 };
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'belongs to' AND spelling_reason = 'correction'
  ...
  console.log(`[${new Date().toISOString()}] belongs-to/correction migration complete in ${elapsed}s`);
}
