...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting belongs-to/recombination migration...`);
  ...
  if (reasonRows.length !== 1) {
    console.error(`  FATAL: expected exactly one namechange_reasons row for reason='recombination'/edge_class='name', got ${reasonRows.length}`);
    process.exit(1);
  }
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'belongs to' AND spelling_reason = 'recombination'
  ...
  console.log(`[${new Date().toISOString()}] belongs-to/recombination migration complete in ${elapsed}s`);
}
