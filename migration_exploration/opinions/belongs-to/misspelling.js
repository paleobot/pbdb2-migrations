// Pair: status = 'belongs to', spelling_reason = 'misspelling' (6,983 rows).
//
// Field mapping confirmed against the Classic UI (2026-08-19) -- back to Pair 1's
// direction, reversed from Pairs 2-4 (recombination/rank-change/correction):
...
// Since both outputs share the same subject field here, child_spelling_no is
// resolved once and reused, unlike Pairs 2-4 where the two outputs' subjects differed.
//
// Every row emits BOTH a name_opinions lineage row (reason 'misspelling',
// never_accepted=true in the dictionary -- the §3.1 Amphymenium/Amphimenium
// discriminator case) and an assignment_opinions containment row, unconditionally
// (ledger model -- no ranking; see the boundary note in taxa-opinions-migration-
// mapping.md). The two emissions are resolved and skipped independently.
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting belongs-to/misspelling migration...`);
  ...
  if (reasonRows.length !== 1) {
    console.error(`  FATAL: expected exactly one namechange_reasons row for reason='misspelling'/edge_class='lineage', got ${reasonRows.length}`);
    process.exit(1);
  }
  const misspellingReasonId = reasonRows[0].id;
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'belongs to' AND spelling_reason = 'misspelling'
  ...
  try {
    for await (const src of stream) {
      ...
      // Shared prerequisite: child_spelling_no is the subject of BOTH outputs here.
  ...
  const totalAssignSkipped = childSpellingUnresolved + Object.values(assignSkip).reduce((a, b) => a + b, 0);
  const totalLineageSkipped = childSpellingUnresolved + Object.values(lineageSkip).reduce((a, b) => a + b, 0);
  ...
  console.log(`[${new Date().toISOString()}] belongs-to/misspelling migration complete in ${elapsed}s`);
}
