...
async function main() {
  ...
  const { rows: reasonRows } = await pg.query(
    `SELECT id FROM dictionaries.namechange_reasons WHERE reason = 'historical misspelling' AND edge_class = 'lineage'`,
  );
  if (reasonRows.length !== 1) {
    console.error(`  FATAL: expected exactly one namechange_reasons row for reason='historical misspelling'/edge_class='lineage', got ${reasonRows.length}`);
    process.exit(1);
  }
    ...
    for (let i = 0; i < lineageRows.length; i += INSERT_BATCH_SIZE) {
      ...
      for (const r of batch) {
        ...
        params.push(
          ...
          r.targetPermid, historicalMisspellingReasonId, 'lineage', null, null, null, null,
