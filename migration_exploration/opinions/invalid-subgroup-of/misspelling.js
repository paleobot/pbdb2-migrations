// Pair: status = 'invalid subgroup of', spelling_reason = 'misspelling' (8 rows).
//
// Per Q3's resolution: dual emission -- concept edge (reason='invalid subgroup',
// objective=NULL, §5.2) + lineage edge (reason='misspelling', curatorial-provenance
// token), both into name_opinions.
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting invalid-subgroup-of/misspelling migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'invalid subgroup of' AND spelling_reason = 'misspelling'
  ...
  try {
    for await (const src of stream) {
      ...
      if (!referenceId) {
        ...
        anomalyLog.log(src.opinion_no, 'name_opinions', 'skip', 'orphan_reference', `concept (invalid subgroup) edge skipped: reference_no=${src.reference_no} not found in migrated refs`);
      ...
      if (!childSpellingPermid) {
        ...
        anomalyLog.log(src.opinion_no, 'name_opinions', 'skip', 'child_spelling_unresolved', `concept (invalid subgroup) edge skipped: child_spelling_no=${src.child_spelling_no} has no migrated permid`);
      ...
      if (!parentSpelling) {
        ...
        anomalyLog.log(src.opinion_no, 'name_opinions', 'skip', 'parent_spelling_zero', 'concept (invalid subgroup) edge skipped: parent_spelling_no is 0');
      } else {
        ...
        if (!targetPermid) {
          ...
          anomalyLog.log(src.opinion_no, 'name_opinions', 'skip', 'parent_spelling_orphan', `concept (invalid subgroup) edge skipped: parent_spelling_no=${src.parent_spelling_no} has no migrated permid`);
          ...
          anomalyLog.log(src.opinion_no, 'name_opinions', 'skip', 'self_reference', `concept (invalid subgroup) edge skipped: child_spelling_no == parent_spelling_no (${src.child_spelling_no})${childNo === Number(src.parent_no) ? ` -- child_no == parent_no (${src.child_no}) too, a same-taxon self-reference opinion` : ''}`);
        } else {
          conceptRows.push({
  ...
  console.log(`  Wrote ${anomalyCount} anomaly rows to opinions/invalid-subgroup-of/anomalies.csv`);
  ...
  console.log(`[${new Date().toISOString()}] invalid-subgroup-of/misspelling migration complete in ${elapsed}s`);
}
