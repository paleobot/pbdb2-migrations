// Pair: status = 'invalid subgroup of', spelling_reason = 'rank change' (43 rows).
//
// Per Q3's resolution: dual emission -- concept edge (reason='invalid subgroup',
// objective=NULL, §5.2) + lineage edge (reason='reranked'), both into name_opinions.
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting invalid-subgroup-of/rank-change migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'invalid subgroup of' AND spelling_reason = 'rank change'
  ...
  console.log(`[${new Date().toISOString()}] invalid-subgroup-of/rank-change migration complete in ${elapsed}s`);
}
