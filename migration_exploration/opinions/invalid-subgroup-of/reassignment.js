// Pair: status = 'invalid subgroup of', spelling_reason = 'reassignment' (2 rows).
//
// Per Q3's resolution: dual emission -- concept edge (reason='invalid subgroup',
// objective=NULL, §5.2) + lineage edge (reason='assignment'), both into name_opinions.
// Last of the six invalid-subgroup-of spelling_reason variants.
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting invalid-subgroup-of/reassignment migration...`);
  ...
  const { rows: lineageReasonRows } = await pg.query(
    `SELECT id FROM dictionaries.namechange_reasons WHERE reason = 'assignment' AND edge_class = 'name'`,
  );
  if (lineageReasonRows.length !== 1) {
    console.error(`  FATAL: expected exactly one namechange_reasons row for reason='assignment'/edge_class='name', got ${lineageReasonRows.length}`);
    process.exit(1);
  }
  const assignmentReasonId = lineageReasonRows[0].id;
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'invalid subgroup of' AND spelling_reason = 'reassignment'
  ...
  try {
    ...
    for (let i = 0; i < lineageRows.length; i += INSERT_BATCH_SIZE) {
      ...
      for (const r of batch) {
        ...
        params.push(
          r.permid, r.authorizerPersonId, r.entererPersonId, null, r.subjectPermid,
          r.targetPermid, assignmentReasonId, 'name', null, null, null, null,
          r.referenceId, r.publicationYear,
          r.attribution === null ? null : JSON.stringify(r.attribution), r.evidence, false,
        );
        p += 17;
  ...
  console.log(`[${new Date().toISOString()}] invalid-subgroup-of/reassignment migration complete in ${elapsed}s`);
}
