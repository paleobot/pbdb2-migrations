// Pair: status = 'invalid subgroup of', spelling_reason = 'recombination' (28 rows).
//
// Per Q3's resolution: dual emission -- concept edge (reason='invalid subgroup',
// objective=NULL, §5.2) + lineage edge (reason='recombination'), both into name_opinions.
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting invalid-subgroup-of/recombination migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'invalid subgroup of' AND spelling_reason = 'recombination'
  ...
  console.log(`[${new Date().toISOString()}] invalid-subgroup-of/recombination migration complete in ${elapsed}s`);
}
