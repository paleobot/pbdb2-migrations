// Pair: status = 'subjective synonym of', spelling_reason = 'correction' (399 rows).
//
...
//   lineage (correction) edge: subject_permid = permid(child_spelling_no), target_permid = permid(child_no)
// Only the lineage reason token differs from pair 8: 'correction'.
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting subjective-synonym-of/correction migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'subjective synonym of' AND spelling_reason = 'correction'
  ...
  try {
    for await (const src of stream) {
      ...
      // ---- lineage edge: subject = child_no, target = child_spelling_no ----
      ...
      if (!childNoPermid) {
        ...
        anomalyLog.log(src.opinion_no, 'name_opinions', 'skip', 'self_reference', `lineage (correction) edge skipped: child_spelling_no == child_no (${src.child_no}) despite spelling_reason='correction' -- row carries no actual spelling deviation`);
      } else {
        lineageRows.push({
          permid: uuidv7(),
          authorizerPersonId,
  ...
  for (const [k, v] of Object.entries(conceptSkip)) console.log(`    ${k}: ${v}`);
  ...
  console.log(`[${new Date().toISOString()}] subjective-synonym-of/correction migration complete in ${elapsed}s`);
}
