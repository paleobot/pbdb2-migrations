// Pair: status = 'subjective synonym of', spelling_reason = 'rank change' (880 rows).
//
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting subjective-synonym-of/rank-change migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'subjective synonym of' AND spelling_reason = 'rank change'
  ...
  try {
    for await (const src of stream) {
      ...
      if (!childNoPermid) {
        ...
        anomalyLog.log(src.opinion_no, 'name_opinions', 'skip', 'self_reference', `lineage (rank change) edge skipped: child_spelling_no == child_no (${src.child_no}) despite spelling_reason='rank change' -- row carries no actual spelling deviation`);
      } else {
        lineageRows.push({
          permid: uuidv7(),
          authorizerPersonId,
  ...
  console.log(`[${new Date().toISOString()}] subjective-synonym-of/rank-change migration complete in ${elapsed}s`);
}
