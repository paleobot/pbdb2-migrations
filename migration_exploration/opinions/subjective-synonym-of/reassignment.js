// Pair: status = 'subjective synonym of', spelling_reason = 'reassignment' (4 rows).
//
...
//   lineage (reassignment) edge: subject_permid = permid(child_spelling_no), target_permid = permid(child_no)
// reason = 'assignment' (§6.1 crosswalk for spelling_reason='reassignment'). Last of
// the six subjective-synonym-of spelling_reason variants.
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting subjective-synonym-of/reassignment migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'subjective synonym of' AND spelling_reason = 'reassignment'
  ...
  console.log(`[${new Date().toISOString()}] subjective-synonym-of/reassignment migration complete in ${elapsed}s`);
}
