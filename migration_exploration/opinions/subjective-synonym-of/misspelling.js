// Pair: status = 'subjective synonym of', spelling_reason = 'misspelling' (320 rows).
//
// Per Q3's resolution (2026-08-19): dual emission -- one concept edge, one lineage
// edge, both into name_opinions. Field mappings (universal rule, §4):
//   concept (synonym) edge: subject_permid = permid(child_spelling_no), target_permid = permid(parent_spelling_no)
//   lineage (misspelling) edge: subject_permid = permid(child_spelling_no), target_permid = permid(child_no)
// reason = 'misspelling' (never_accepted=true) -- the curatorial-provenance token,
// since this is spelling_reason='misspelling' noticed on an unrelated (synonymy)
// opinion, not the dedicated 'misspelling of' status (see create_new.sql's
// namechange_reasons header and mapping doc §6.1).
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting subjective-synonym-of/misspelling migration...`);
  ...
  if (conceptReasonRows.length !== 1) {
    console.error(`  FATAL: expected exactly one namechange_reasons row for reason='junior synonym'/edge_class='concept', got ${conceptReasonRows.length}`);
    process.exit(1);
  }
  const juniorSynonymReasonId = conceptReasonRows[0].id;
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'subjective synonym of' AND spelling_reason = 'misspelling'
    ...
    for await (const src of stream) {
      ...
      if (!parentSpelling) {
        ...
        if (!targetPermid) {
          ...
          anomalyLog.log(src.opinion_no, 'name_opinions', 'skip', 'self_reference', `concept (junior synonym) edge skipped: child_spelling_no == parent_spelling_no (${src.child_spelling_no})`);
        } else {
          conceptRows.push({
            permid: uuidv7(),
            authorizerPersonId,
      ...
      // ---- lineage edge: subject = child_spelling_no, target = child_no ----
  ...
  console.log(`[${new Date().toISOString()}] subjective-synonym-of/misspelling migration complete in ${elapsed}s`);
}
