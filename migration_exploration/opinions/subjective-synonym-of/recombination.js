// Pair: status = 'subjective synonym of', spelling_reason = 'recombination' (2,816 rows).
//
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting subjective-synonym-of/recombination migration...`);
  ...
  let sourceRows = 0;
  // orphan_reference and child_spelling_unresolved are shared prerequisites (both
  // outputs depend on them) -- counted once, not per-output.
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'subjective synonym of' AND spelling_reason = 'recombination'
  ...
  try {
    for await (const src of stream) {
      ...
      if (!childNoPermid) {
        ...
        anomalyLog.log(src.opinion_no, 'name_opinions', 'skip', 'self_reference', `lineage (recombination) edge skipped: child_spelling_no == child_no (${src.child_no}) despite spelling_reason='recombination' -- row carries no actual spelling deviation`);
      } else {
        lineageRows.push({
          permid: uuidv7(),
          authorizerPersonId,
  ...
  console.log(`[${new Date().toISOString()}] subjective-synonym-of/recombination migration complete in ${elapsed}s`);
}
