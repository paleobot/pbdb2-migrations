// Pair: status = 'objective synonym of', spelling_reason = 'misspelling' (6 rows).
//
// Per Q3's resolution: dual emission -- concept edge + lineage edge, both into
// name_opinions. Same shape as Pair 11 (subjective-synonym-of/misspelling), with
// objective=true instead of false. reason='misspelling' (curatorial-provenance token).
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting objective-synonym-of/misspelling migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'objective synonym of' AND spelling_reason = 'misspelling'
    ...
    for (let i = 0; i < conceptRows.length; i += INSERT_BATCH_SIZE) {
      ...
      for (const r of batch) {
        ...
        params.push(
          ...
          r.targetPermid, juniorSynonymReasonId, 'concept', true, null, null, null,
  ...
  console.log(`[${new Date().toISOString()}] objective-synonym-of/misspelling migration complete in ${elapsed}s`);
}
