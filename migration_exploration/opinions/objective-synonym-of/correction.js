// Pair: status = 'objective synonym of', spelling_reason = 'correction' (15 rows).
//
// Per Q3's resolution: dual emission -- concept edge + lineage edge, both into
// name_opinions. Same shape as Pair 10 (subjective-synonym-of/correction), with
// objective=true instead of false.
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting objective-synonym-of/correction migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'objective synonym of' AND spelling_reason = 'correction'
  ...
  console.log(`[${new Date().toISOString()}] objective-synonym-of/correction migration complete in ${elapsed}s`);
}
