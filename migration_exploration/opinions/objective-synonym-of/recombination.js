// Pair: status = 'objective synonym of', spelling_reason = 'recombination' (36 rows).
//
// Per Q3's resolution: dual emission -- concept edge + lineage edge, both into
// name_opinions. Same shape as Pair 8 (subjective-synonym-of/recombination), with
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting objective-synonym-of/recombination migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'objective synonym of' AND spelling_reason = 'recombination'
  ...
  console.log(`[${new Date().toISOString()}] objective-synonym-of/recombination migration complete in ${elapsed}s`);
}
