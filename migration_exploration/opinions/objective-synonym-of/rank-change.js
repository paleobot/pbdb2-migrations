// Pair: status = 'objective synonym of', spelling_reason = 'rank change' (37 rows).
//
// Per Q3's resolution: dual emission -- concept edge + lineage edge, both into
// name_opinions. Same shape as Pair 9 (subjective-synonym-of/rank-change), with
...
async function main() {
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'objective synonym of' AND spelling_reason = 'rank change'
  ...
  console.log(`[${new Date().toISOString()}] objective-synonym-of/rank-change migration complete in ${elapsed}s`);
}
