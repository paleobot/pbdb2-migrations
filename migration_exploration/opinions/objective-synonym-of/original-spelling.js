// Pair: status = 'objective synonym of', spelling_reason = 'original spelling' (1,152 rows).
//
// Field mapping matches Pair 7 (subjective synonym of / original spelling), pre-existing
// migrate-synonymy-opinions.js convention: subject_permid = permid(child_spelling_no),
// target_permid = permid(parent_spelling_no). Single-output pair -- no assignment_opinions.
// Only difference from Pair 7: objective = true (same type specimen defines both names).
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting objective-synonym-of/original-spelling migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'objective synonym of' AND spelling_reason = 'original spelling'
  ...
  try {
    for await (const src of stream) {
      ...
      if (child === parent) {
        ...
        anomalyLog.log(src.opinion_no, 'name_opinions', 'skip', 'self_reference', `concept (junior synonym) edge skipped: child_spelling_no == parent_spelling_no (${src.child_spelling_no})${Number(src.child_no) === Number(src.parent_no) ? ` -- child_no == parent_no (${src.child_no}) too, a same-taxon self-synonymy opinion` : ''}`);
        continue;
      }
  ...
  console.log(`[${new Date().toISOString()}] objective-synonym-of/original-spelling migration complete in ${elapsed}s`);
}
