// Pair: status = 'replaced by', spelling_reason = 'misspelling' (8 rows).
//
// Per Q3's resolution: dual emission -- concept edge (reason='replaced by',
// objective=NULL) + lineage edge (reason='misspelling', curatorial-provenance token),
// both into name_opinions. Last of the five replaced-by spelling_reason variants.
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting replaced-by/misspelling migration...`);
  ...
  if (conceptReasonRows.length !== 1) {
    console.error(`  FATAL: expected exactly one namechange_reasons row for reason='replaced by'/edge_class='concept', got ${conceptReasonRows.length}`);
    process.exit(1);
  }
  const replacedByReasonId = conceptReasonRows[0].id;
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'replaced by' AND spelling_reason = 'misspelling'
    ...
    for await (const src of stream) {
      ...
      if (!parentSpelling) {
        ...
        if (!targetPermid) {
          ...
          const note = childNo === Number(src.parent_no)
            ? ` -- child_no == parent_no (${src.child_no}) too, a same-taxon self-reference opinion`
            : ` -- child_no (${src.child_no}) != parent_no (${src.parent_no}), but the corrected child_spelling_no converges onto the replacement target's identity`;
          anomalyLog.log(src.opinion_no, 'name_opinions', 'skip', 'self_reference', `concept (replaced by) edge skipped: child_spelling_no == parent_spelling_no (${src.child_spelling_no})${note}`);
        } else {
          conceptRows.push({
  ...
  console.log(`[${new Date().toISOString()}] replaced-by/misspelling migration complete in ${elapsed}s`);
}
