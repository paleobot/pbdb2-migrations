// Pair: status = 'replaced by', spelling_reason = 'correction' (50 rows).
//
// Per Q3's resolution: dual emission -- concept edge (reason='replaced by',
// objective=NULL) + lineage edge (reason='correction'), both into name_opinions.
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting replaced-by/correction migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'replaced by' AND spelling_reason = 'correction'
  ...
  console.log(`[${new Date().toISOString()}] replaced-by/correction migration complete in ${elapsed}s`);
}
