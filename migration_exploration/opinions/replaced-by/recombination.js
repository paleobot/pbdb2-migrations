// Pair: status = 'replaced by', spelling_reason = 'recombination' (160 rows).
//
// Per Q3's resolution: dual emission -- concept edge (reason='replaced by',
// objective=NULL) + lineage edge (reason='recombination'), both into name_opinions.
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting replaced-by/recombination migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'replaced by' AND spelling_reason = 'recombination'
  ...
  console.log(`[${new Date().toISOString()}] replaced-by/recombination migration complete in ${elapsed}s`);
}
