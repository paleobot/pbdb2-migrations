// Pair: status = 'replaced by', spelling_reason = 'rank change' (96 rows).
//
// Per Q3's resolution: dual emission -- concept edge (reason='replaced by',
// objective=NULL) + lineage edge (reason='reranked'), both into name_opinions.
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting replaced-by/rank-change migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'replaced by' AND spelling_reason = 'rank change'
  ...
  console.log(`[${new Date().toISOString()}] replaced-by/rank-change migration complete in ${elapsed}s`);
}
