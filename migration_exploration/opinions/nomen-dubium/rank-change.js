// Pair: status = 'nomen dubium', spelling_reason = 'rank change' (8 rows).
//
...
// (subject=child_spelling_no, target=child_no, reason='reranked'). Last of the five
// nomen-dubium spelling_reason variants.
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting nomen-dubium/rank-change migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'nomen dubium' AND spelling_reason = 'rank change'
  ...
  console.log(`[${new Date().toISOString()}] nomen-dubium/rank-change migration complete in ${elapsed}s`);
}
