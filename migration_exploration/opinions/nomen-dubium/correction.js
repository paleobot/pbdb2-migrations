// Pair: status = 'nomen dubium', spelling_reason = 'correction' (73 rows).
//
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting nomen-dubium/correction migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'nomen dubium' AND spelling_reason = 'correction'
  ...
  console.log(`[${new Date().toISOString()}] nomen-dubium/correction migration complete in ${elapsed}s`);
}
