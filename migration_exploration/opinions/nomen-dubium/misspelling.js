// Pair: status = 'nomen dubium', spelling_reason = 'misspelling' (91 rows).
//
// Per Q3's resolution: dual emission -- validity_opinions testimony (subject=
// child_spelling_no, status='nomen dubium', no target) + name_opinions lineage edge
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting nomen-dubium/misspelling migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'nomen dubium' AND spelling_reason = 'misspelling'
  ...
  console.log(`[${new Date().toISOString()}] nomen-dubium/misspelling migration complete in ${elapsed}s`);
}
