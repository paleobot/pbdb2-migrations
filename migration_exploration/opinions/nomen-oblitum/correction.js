// Pair: status = 'nomen oblitum', spelling_reason = 'correction' (3 rows).
//
// Same structure as recombination.js: targeted/untargeted parent_no branch (Q4) +
// Q3 dual-emission lineage edge (reason='correction'), orthogonal to the branch.
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting nomen-oblitum/correction migration...`);
  ...
  const { rows: lineageReasonRows } = await pg.query(
    `SELECT id FROM dictionaries.namechange_reasons WHERE reason = 'correction' AND edge_class = 'name'`,
  );
  if (lineageReasonRows.length !== 1) {
    console.error(`  FATAL: expected exactly one namechange_reasons row for reason='correction'/edge_class='name', got ${lineageReasonRows.length}`);
    process.exit(1);
  }
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'nomen oblitum' AND spelling_reason = 'correction'
  ...
  try {
    for await (const src of stream) {
      ...
      if (!childSpellingPermid) {
        ...
        anomalyLog.log(src.opinion_no, 'name_opinions', 'skip', 'child_spelling_unresolved', `lineage (correction) edge skipped: child_spelling_no=${src.child_spelling_no} has no migrated permid`);
        continue;
      }
      if (!referenceId) {
        lineageSkip.orphan_reference++;
        logSkip(`opinion_no=${src.opinion_no} lineage_orphan_reference reference_no=${src.reference_no}`);
        anomalyLog.log(src.opinion_no, 'name_opinions', 'skip', 'orphan_reference', `lineage (correction) edge skipped: reference_no=${src.reference_no} not found in migrated refs`);
        continue;
      }
      ...
      if (!childNoPermid) {
        ...
        anomalyLog.log(src.opinion_no, 'name_opinions', 'skip', 'child_no_unresolved', `lineage (correction) edge skipped: child_no=${src.child_no} has no migrated permid`);
        continue;
      }
      if (child === childNo) {
        lineageSkip.self_reference++;
        logSkip(`opinion_no=${src.opinion_no} lineage_self_reference taxon=${src.child_spelling_no}`);
        anomalyLog.log(src.opinion_no, 'name_opinions', 'skip', 'self_reference', `lineage (correction) edge skipped: child_spelling_no == child_no (${src.child_spelling_no}) despite spelling_reason='correction' -- row carries no actual spelling deviation`);
        continue;
      }
  ...
  try {
    ...
    for (let i = 0; i < lineageRows.length; i += INSERT_BATCH_SIZE) {
      ...
      for (const r of batch) {
        ...
        params.push(
          r.permid, r.authorizerPersonId, r.entererPersonId, null, r.subjectPermid,
          r.targetPermid, correctionReasonId, 'name', null, null, null, null,
          r.referenceId, r.publicationYear,
          r.attribution === null ? null : JSON.stringify(r.attribution), r.evidence, false,
        );
        p += 17;
  ...
  console.log(`[${new Date().toISOString()}] nomen-oblitum/correction migration complete in ${elapsed}s`);
}
