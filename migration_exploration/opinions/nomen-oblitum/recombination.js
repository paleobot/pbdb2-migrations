// Pair: status = 'nomen oblitum', spelling_reason = 'recombination' (6 rows).
//
// Combines both special cases: the targeted/untargeted parent_no branch (Q4, see
// original-spelling.js) AND the Q3 dual-emission lineage edge (since spelling_reason
// != 'original spelling'). Per row: targeted -> concept fold; untargeted -> validity
// testimony; either way, ALSO emit a lineage edge (subject=child_spelling_no,
// target=child_no, reason='recombination') since that's orthogonal to the parent_no
// branch entirely.
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting nomen-oblitum/recombination migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'nomen oblitum' AND spelling_reason = 'recombination'
  ...
  try {
    for await (const src of stream) {
      ...
      // ---- targeted -> concept fold / untargeted -> validity testimony ----
      if (targeted) {
        targetedCount++;
      ...
      // ---- lineage edge (orthogonal to targeted/untargeted): subject = child_spelling_no, target = child_no ----
  ...
  console.log(`[${new Date().toISOString()}] nomen-oblitum/recombination migration complete in ${elapsed}s`);
}
