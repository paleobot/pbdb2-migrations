...
async function main() {
  ...
  const { rows: lineageReasonRows } = await pg.query(
    `SELECT id FROM dictionaries.namechange_reasons WHERE reason = 'misspelling' AND edge_class = 'name'`,
  );
  if (lineageReasonRows.length !== 1) {
    console.error(`  FATAL: expected exactly one namechange_reasons row for reason='misspelling'/edge_class='name', got ${lineageReasonRows.length}`);
    process.exit(1);
  }
  ...
  try {
    ...
    for (let i = 0; i < lineageRows.length; i += INSERT_BATCH_SIZE) {
      ...
      for (const r of batch) {
        ...
        params.push(
          r.permid, r.authorizerPersonId, r.entererPersonId, null, r.subjectPermid,
          r.targetPermid, misspellingReasonId, 'name', null, null, null, null,
          r.referenceId, r.publicationYear,
          r.attribution === null ? null : JSON.stringify(r.attribution), r.evidence, false,
        );
        p += 17;
