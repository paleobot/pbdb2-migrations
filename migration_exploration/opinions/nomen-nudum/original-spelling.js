// Pair: status = 'nomen nudum', spelling_reason = 'original spelling' (2,393 rows).
//
// validity_opinions testimony (§5.2): subject_permid = permid(child_spelling_no),
// nomenclatural_status_id = 'nomen nudum' (bars_candidacy=true -- the one status with a
// real derive() effect: it can bar its own subject from winning its lineage's
// accepted-spelling contest). No target (§5.2). Single-output pair -- no
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting nomen-nudum/original-spelling migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'nomen nudum' AND spelling_reason = 'original spelling'
  ...
  console.log(`[${new Date().toISOString()}] nomen-nudum/original-spelling migration complete in ${elapsed}s`);
}
