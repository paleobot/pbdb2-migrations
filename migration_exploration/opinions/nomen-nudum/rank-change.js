// Pair: status = 'nomen nudum', spelling_reason = 'rank change' (2 rows).
//
...
// reason='reranked'). Last of the five nomen-nudum spelling_reason variants.
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting nomen-nudum/rank-change migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'nomen nudum' AND spelling_reason = 'rank change'
  ...
  console.log(`[${new Date().toISOString()}] nomen-nudum/rank-change migration complete in ${elapsed}s`);
}
