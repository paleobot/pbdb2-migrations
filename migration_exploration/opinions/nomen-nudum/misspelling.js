// Pair: status = 'nomen nudum', spelling_reason = 'misspelling' (36 rows).
//
...
// reason='misspelling', curatorial token).
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting nomen-nudum/misspelling migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'nomen nudum' AND spelling_reason = 'misspelling'
  ...
  console.log(`[${new Date().toISOString()}] nomen-nudum/misspelling migration complete in ${elapsed}s`);
}
