// Pair: status = 'nomen nudum', spelling_reason = 'correction' (11 rows).
//
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting nomen-nudum/correction migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'nomen nudum' AND spelling_reason = 'correction'
  ...
  console.log(`[${new Date().toISOString()}] nomen-nudum/correction migration complete in ${elapsed}s`);
}
