// Pair: status = 'nomen vanum', spelling_reason = 'misspelling' (6 rows).
//
// Per Q3's resolution: dual emission -- validity_opinions testimony (subject=
// child_spelling_no, status='nomen vanum', no target) + name_opinions lineage edge
// (subject=child_spelling_no, target=child_no, reason='misspelling', curatorial token).
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting nomen-vanum/misspelling migration...`);
  ...
  const { rows: statusRows } = await pg.query(
    `SELECT id FROM dictionaries.nomenclatural_statuses WHERE status = 'nomen vanum'`,
  );
  if (statusRows.length !== 1) {
    console.error(`  FATAL: expected exactly one nomenclatural_statuses row for status='nomen vanum', got ${statusRows.length}`);
    process.exit(1);
  }
  const nomenVanumStatusId = statusRows[0].id;
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'nomen vanum' AND spelling_reason = 'misspelling'
  ...
  try {
    ...
    for (let i = 0; i < validityRows.length; i += INSERT_BATCH_SIZE) {
      ...
      for (const r of batch) {
        ...
        params.push(
          r.permid, r.authorizerPersonId, r.entererPersonId, r.subjectPermid, nomenVanumStatusId,
  ...
  console.log(`[${new Date().toISOString()}] nomen-vanum/misspelling migration complete in ${elapsed}s`);
}
