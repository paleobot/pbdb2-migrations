// Pair: status = 'nomen vanum', spelling_reason = 'reassignment' (1 row).
//
...
// (subject=child_spelling_no, target=child_no, reason='assignment'). Last of the five
// nomen-vanum spelling_reason variants, and the last of all 48 pairs.
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting nomen-vanum/reassignment migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'nomen vanum' AND spelling_reason = 'reassignment'
  ...
  console.log(`[${new Date().toISOString()}] nomen-vanum/reassignment migration complete in ${elapsed}s`);
}
