// Pair: status = 'nomen vanum', spelling_reason = 'correction' (4 rows).
//
...
// (subject=child_spelling_no, target=child_no, reason='correction').
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting nomen-vanum/correction migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'nomen vanum' AND spelling_reason = 'correction'
  ...
  console.log(`[${new Date().toISOString()}] nomen-vanum/correction migration complete in ${elapsed}s`);
}
