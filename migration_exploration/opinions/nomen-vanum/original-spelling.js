// Pair: status = 'nomen vanum', spelling_reason = 'original spelling' (509 rows).
//
// validity_opinions testimony (§5.2): subject_permid = permid(child_spelling_no),
// nomenclatural_status_id = 'nomen vanum' (bars_candidacy=false, no derive() effect --
// same reasoning as nomen dubium: doubt about a poor type specimen is not an act of
// invalidation). No target (§5.2). Single-output pair -- no assignment_opinions, no
// lineage (original spelling).
...
async function main() {
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'nomen vanum' AND spelling_reason = 'original spelling'
  ...
  console.log(`[${new Date().toISOString()}] nomen-vanum/original-spelling migration complete in ${elapsed}s`);
}
