// Pair: status = 'nomen vanum', spelling_reason = 'recombination' (49 rows).
//
...
// (subject=child_spelling_no, target=child_no, reason='recombination').
...
async function main() {
  ...
  console.log(`[${startTime.toISOString()}] Starting nomen-vanum/recombination migration...`);
  ...
  const stream = conn.connection.query(`
    ...
    FROM opinions
    WHERE status = 'nomen vanum' AND spelling_reason = 'recombination'
  ...
  console.log(`[${new Date().toISOString()}] nomen-vanum/recombination migration complete in ${elapsed}s`);
}
