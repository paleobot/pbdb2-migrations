...
authorities pass mints **roots only** (already implemented in `migrate-authority-opinions.js`); `name`
...
> - This pass (now `migrate-authority-opinions.js`) mints **ROOT rows only**, unconditionally, one per
