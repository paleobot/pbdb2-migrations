...
-- The first and highest-priority sort key in derive()'s winner selection.
-- Legacy `opinions.basis` is a 5-value enum, NOT a boolean: a boolean cannot
-- express the ordering the ranking depends on. 298,470 of 998,565 legacy
-- opinions (30%) have basis IS NULL and fall back to the reference's basis,
-- so evidence_id must be nullable.
CREATE TABLE dictionaries.evidence_bases (
    id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    basis text NOT NULL,
    reliability integer NOT NULL   -- derive() orders on this DESC, first key
);
INSERT INTO dictionaries.evidence_bases (basis, reliability) VALUES
    ('stated with evidence',    40),
    ('stated without evidence', 30),
    ('implied',                 20),
    ('second hand',             10),
    -- Migration-synthesized opinions sit at the floor so a real published
    -- opinion always outranks them. Without this they would win on `pubyr`,
    -- since a naming reference is by definition the oldest one available.
    ('synthesized',              0);
...
CREATE TABLE name_opinions (
    ...
    pubyr text,          -- overrides refs publicationYear when second-hand
    attribution jsonb,   -- optional; same shape as payloadSchemas/authority.schema.js
    evidence_id integer REFERENCES dictionaries.evidence_bases("id"),  -- NULL → fall back to the reference
...
CREATE TABLE rank_opinions (
    ...
    pubyr text,
    attribution jsonb,
    evidence_id integer REFERENCES dictionaries.evidence_bases("id"),
