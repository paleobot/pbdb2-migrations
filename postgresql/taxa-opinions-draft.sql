...
--
--     EXCEPTION: the six opinion tables are versioned but deliberately do NOT
...
CREATE TABLE rank_opinions (
    ...
    created_at timestamptz NOT NULL DEFAULT NOW(),
    removed boolean,
    preceded_by_id bigint REFERENCES rank_opinions("id"),
    succeeded_by_id bigint REFERENCES rank_opinions("id")
);
...
CREATE INDEX name_opinions_senior_idx        ON name_opinions (senior_permid)        WHERE succeeded_by_id IS NULL;
CREATE INDEX rank_opinions_subject_idx       ON rank_opinions (subject_permid)       WHERE succeeded_by_id IS NULL;
...
CREATE INDEX rank_opinions_permid_head_idx       ON rank_opinions (permid)       WHERE succeeded_by_id IS NULL;
